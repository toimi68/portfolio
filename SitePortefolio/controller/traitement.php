...
if(empty($_POST)){
    ...
    if(!isset($_POST['prenom']) || strlen($_POST['prenom'])< 3 || strlen($_Post['prenom']) > 30){
        $contactError .= '<div class="alert alert-warning text-danger"> Indiquez votre prénom (entre 3 et 30 caractères)</div>';
    }
    ...
    if(!isset($_POST['message']) || strlen($_POST['message']) < 3 || strlen($_POST['message']) > 250){
        $contactError .= '<div class="alert alert-warning text-danger">Saisissez votre messages (250 caractères max)</div>';
    }
