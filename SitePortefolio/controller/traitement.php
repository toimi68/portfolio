...
if($_POST && isset($_POST['cptechnology'])){
    ...
    if(empty($msg)){
        ...
        $result = $pdo->prepare("INSERT INTO competences (cptechnology, cplevel) VALUES (:cptechnology, :cplevel)");
        $result->bindParam(':cptechnology',$_POST['cptechnology'], PDO::PARAM_STR);
        $result->bindParam(':cplevel',$_POST['cplevel'], PDO::PARAM_INT);
        $result->execute();
...
//    debugV($_GET);
    ...
    if($req->rowCount() > 0){
        ...
        //$schoolUpdate = $req->fetch(PDO::FETCH_ASSOC);
        $school_update=$req->fetch(PDO::FETCH_ASSOC);
//        extract($school_update);
        //$year = $school_update['sgdate'];
    }
   debugV( $school_update);
}
while($year >= $century){
    if(isset($_GET['action']) && $_GET['action'] == 'update' && isset($_GET['id']) && $_GET['id'] == $school_update['idschooling'] && $school_update['sgdate']
        == $year){
